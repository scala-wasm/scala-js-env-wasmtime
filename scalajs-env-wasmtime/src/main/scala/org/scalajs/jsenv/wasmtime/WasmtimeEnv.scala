...
object WasmtimeEnv {
  ...
  private[wasmtime] def resolveInput(input: Seq[Input]): Path = {
    val wasmComponents = input.collect { case wasm: WasmtimeInput.WasmComponent =>
      wasm.component
    }

    if (wasmComponents.size != input.size) {
      throw new UnsupportedInputException(
        "WasmtimeEnv only supports WasmtimeInput.WasmComponent. " +
          "Do not mix it with other Input implementations."
      )
    }
    if (wasmComponents.size != 1) {
      throw new UnsupportedInputException(
        s"WasmtimeEnv requires exactly one WasmtimeInput.WasmComponent, " +
          s"got ${wasmComponents.size}."
      )
    }
